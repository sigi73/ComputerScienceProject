...
private:
    ...
    void createImageView(VkImage image, VkFormat format, VkImageAspectFlags
    ...
    void createShaderModule(const std::vector<char> &code, VDeleter<VkShaderModule> 
        &shaderModule) {
    ...
    VkFormat findSupportedFormat(const std::vector<VkFormat> &candidates, VkImageTiling
    ...
    void createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling
    ...
    void copyImage(VkImage srcImage, VkImage dstImage, uint32_t width,
        uint32_t height) {
        VkCommandBuffer commandBuffer = beginSingleTimeCommands();
